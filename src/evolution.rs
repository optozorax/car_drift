...
impl NnParameters {
    ...
    pub fn get_nns_autoencoder_len(&self) -> usize {
        if self.use_dirs_autoencoder { self.get_nn_autoencoder_input_len() + self.get_nn_autoencoder_output_len() } else { 0 }
    }
...
pub fn mirror_horizontally_track2((name, mut points): (String, Vec<PointsStorage>)) -> (String, Vec<PointsStorage>) {
    let car_center = 250.;
    let points = points.into_iter().map(|mut storage| {
...
pub fn track_straight_line() -> (String, Vec<PointsStorage>) {
    (
        "straight".to_owned(),
        vec![
        PointsStorage{ is_reward: false, points:
            vec![
                pos2(-8.00, 13.00),
                pos2(5635.54, 246.73),
                pos2(5630.75, 488.32),
        ...
        PointsStorage { is_reward: true, points: 
            vec![
                pos2(516.17, 234.58),
                pos2(1918.37, 265.53),
                pos2(4208.82, 325.74),
...
pub fn track_complex() -> (String, Vec<PointsStorage>) {
    (
        "complex".to_owned(),
        vec![
        PointsStorage{ is_reward: false, points:vec![
            pos2(-11.43, -2.86),
            pos2(2150.00, -1.43),
        ...
        PointsStorage { is_reward: true, points: vec![
            pos2(594.50, 289.83),
            pos2(1211.28, 297.60),
            pos2(1820.29, 317.04),
            pos2(2472.05, 175.80),
...
impl RewardPathProcessor {
    ...
    fn draw(&self, point: Pos2, painter: &Painter, to_screen: &RectTransform) {
        let projection1 = self.rewards.windows(2).enumerate().map(|(pos, x)| {
            let (a, b) = (x[0].center, x[1].center);
            let (res, t) = project_to_segment(point, a, b);
            (pos, res, (point - res).length(), t)
        }).min_by(|a, b| a.2.partial_cmp(&b.2).unwrap()).unwrap_or_default().1;
...
impl NnProcessorAutoencoder {
    ...
    pub fn new(params: &[f32], nn_params: NnParameters) -> Self {
        let (params_input, params_output) = params.split_at(nn_params.get_nn_autoencoder_input_len());
        Self {
            input: vec![0.; nn_params.dirs_size],
            output: vec![0.; nn_params.autoencoder_exits],
            nn_input: NeuralNetwork::new_params(nn_params.get_nn_autoencoder_input_sizes(), params_input),
            nn_output: NeuralNetwork::new_params(nn_params.get_nn_autoencoder_output_sizes(), params_output),
            autoencoder_loss: 0.,
            calc_counts: 0,
            params: nn_params,
        }
    ...
    pub fn process(
        ...
        if self.params.pass_dirs_diff {
            for (prev, current) in self.prev_dirs.iter_mut().zip(dirs.iter()) {
                *input_values_iter.next().unwrap() = convert_dir(&self.params, *prev) - convert_dir(&self.params, *current);
                *prev = *current;
            }
        }
        ...
        if self.params.pass_current_segment {
            for i in 0..self.params.max_segments {
                *input_values_iter.next().unwrap() = if i as f32 <= current_segment_f32 && current_segment_f32 < (i + 1) as f32 {
...
fn update_two_nearest(current: (Option<f32>, Option<f32>), new_value: Option<f32>) -> (Option<f32>, Option<f32>) {
...
impl CarSimulation {
    ...
    pub fn reset(&mut self) {
        ...
        self.dirs_values_second_layer.iter_mut().for_each(|x| *x = None);
    ...
    pub fn step(
        ...
        get_input: &mut impl FnMut(f32, f32, f32, &[Option<f32>], &[Option<f32>], &InternalCarValues, f32) -> CarInput,
        drift_observer: &mut impl FnMut(usize, Vec2, f32),
        observe_car_forces: &mut impl FnMut(&Car),
    ) -> bool {
        for ((dir, value), value_second_layer) in self.dirs.iter().zip(self.dirs_values.iter_mut()).zip(self.dirs_values_second_layer.iter_mut()) {
            let dir_pos = self.car.from_local_coordinates(*dir);
            let origin = self.car.get_center();
            let mut intersections = (None, None);
            for wall in &self.walls {
                intersections = update_two_nearest(intersections, wall.intersect_ray(origin, dir_pos));
            }
...
pub fn sum_evals(evals: &[TrackEvaluation], params: &SimulationParameters) -> f32 {
    if params.rewards_second_way {
        ...
        let len = evals.len() as f32;
        let tracks_sum = evals.iter().map(|x| x.all_acquired as usize as f32 + x.distance_percent).sum::<f32>();
        let penalty_sum = evals.iter().map(|x| (x.penalty == 0.) as usize as f32 + 1. / (1. + x.penalty)).sum::<f32>();
        let early_finish_reward = evals.iter().map(|x| if x.penalty == 0. {
...
pub fn eval_nn(
    ...
    }) in tracks.into_iter().enumerate()
    {
        ...
        for i in 0..max {
            ...
            for (simple_physics, traction, acceleration_ratio, friction_coef, turn_speed) in (0..physics_max).map(|i| if i == physics_max-1 { 1.0 } else {i as f32 / (physics_max - 1) as f32}.sqrt().sqrt()).map(|s|{ let mut x = default; x.0 = s; x })
            .chain(
                ...
                for i in 0..steps_quota {
                    if simulation.step(
                        ...
                        &params_sim,
                        &mut |_, _, _, _| (),
                        &mut |time, dist, dpenalty, dirs, dirs_second_layer, internals, current_segment_f32| {
                            nn_processor.process(time, dist, dpenalty, dirs, dirs_second_layer, current_segment_f32, internals, &params_sim)
                        },
                        &mut |_, _, _| (),
                        &mut |_| (),
                    ) {
...
pub fn evolve_by_cma_es_custom(
    ...
    for pos in 0..generations_count {
        ...
        if PRINT && (((pos % 10 == 0) && PRINT_EVERY_10) || !PRINT_EVERY_10) {
            ...
            if PRINT_EVALS && (((pos % 10 == 0) && PRINT_EVERY_10_ONLY_EVALS) || !PRINT_EVERY_10_ONLY_EVALS) {
                for i in &evals {
                    println!("{}", i);
                }
            }
...
fn test_params_sim_evolve_simple(params_sim: &SimulationParameters, params_phys: &PhysicsParameters, name: &str) {
    test_params_sim_fn(
        ...
        |params_sim, params_phys, input, population_size, generations_count| {
            evolve_simple_physics(&params_sim, params_phys, &input, population_size, generations_count, 10)
        }
    )
}
...
fn random_input_by_len(len: usize, limit: f32) -> Vec<f32> {
    let mut rng = thread_rng();
    (0..len).map(|_| rng.gen_range(-limit..limit)).collect::<Vec<f32>>()
}
...
fn test_params_sim_fn<F: Fn(&SimulationParameters, &PhysicsParameters, &[f32], usize, usize) -> Vec<EvolveOutputEachStep> + std::marker::Sync>(params_sim: &SimulationParameters, params_phys: &PhysicsParameters, name: &str, f: F) {
    let now = Instant::now();
    let map_lambda = |_| {
        f(params_sim, params_phys, &random_input(params_sim), POPULATION_SIZE, GENERATIONS_COUNT)
    };
...
fn test_params_sim(params_sim: &SimulationParameters, params_phys: &PhysicsParameters, name: &str) {
    let now = Instant::now();
    let map_lambda = |_| {
        evolve_by_cma_es_custom(&params_sim, &params_phys, &random_input(params_sim), params_sim.evolution_population_size, params_sim.evolution_generation_count, Some(params_sim.evolution_distance_to_solution), None)
    };
...
fn eval_tracks_dirs(params_sim: &SimulationParameters, params_phys: &PhysicsParameters, input: &[f32]) -> AllTrackDirs {
    let evals = eval_nn(
        &input,
        &params_phys,
        &params_sim,
    ...
    }) in tracks.into_iter().enumerate()
    {
        ...
        let mut nn_processor = NnProcessor::new(nn_params, params_sim.nn.clone(), params_sim.simulation_simple_physics, track_no);
        let mut simulation =
            CarSimulation::new(Default::default(), walls.clone(), rewards.clone(), &params_sim);
...
#[allow(unused_imports, unused_variables)]
fn evolve_by_bfgs_autoencoder(input: &[f32], all_tracks_dirs: &AllTrackDirs, nn_params: &NnParameters) {
    use argmin::solver::conjugategradient::beta::*;
    use argmin::solver::conjugategradient::*;
    use argmin::solver::gradientdescent::SteepestDescent;
    use argmin::solver::neldermead::*;
...
pub fn evolution() {
    ...
    save_runs(vec![evolve_by_cma_es_custom(&params_sim, &params_phys, &random_input(&params_sim), 50, 100, None, None)], "nn_autoencoder_best_complex");
    ...
    let input = vec![50.82979, 6.8255806, -212.33067, 62.745567, 45.237362, -140.87233, -72.98127, 93.25558, -148.62775, -100.87053, -106.10733, 62.22232, 49.03537, -30.563215, 26.71632, -69.72745, -50.753548, 57.839073, -87.69297, 19.02222, 130.9145, -63.085625, 65.10956, -85.650406, 70.79011, -52.00149, -180.66316, -14.790712, 40.251663, 17.33697, 105.49219, 64.3382, -78.830376, -16.15948, 27.656473, 96.60939, 79.10908, -3.3050725, 55.359814, 37.64303, -3.6646028, 80.42091, -66.87056, 251.49683, 60.166653, 136.15143, -16.154272, -22.46706, -26.938929, 107.08363, -11.486605, 86.18303, -71.15506, 77.64043, -69.64464, 49.550068, -40.087555, 69.021706, 8.264989, -82.418915, 134.31361, -3.7207134, 37.738583, -99.52632, -44.99712, 92.17385, 124.648224, -88.02564, 250.21399, 25.870653, -88.77891, -135.47725, -48.42257, 35.93068, -131.85873, -216.89653, -75.10553, 44.62893, -26.013088, 61.770203, -3.4767735, -65.3946, 57.212772, 77.39033, 21.73365, 26.406633, 19.281778, 76.58683, -80.37626, 71.662224, 111.12739, -60.910496, -25.881977, -115.78931, 63.433617, -69.30505, 43.180832, -68.79547, 27.846312, -29.47661, -169.82027, -2.7010465, 25.906143, 24.204971, -154.87703, -117.11982, 72.440285, -40.214993, -90.72162, -83.748764, -81.2134, -116.93174, 50.3682, -20.83887, 158.06996, 138.1619, 57.19443, -9.595369, -127.12494, -190.78584, 44.887077, -29.397337, 34.447834, -130.67612, -81.49003, 99.291145, 71.481766, 238.19449, -97.65335, 92.890236, -2.9468393, -115.50133, 38.888493, -2.8033266, 35.471237, 64.675995, -36.296505, -9.266836, -119.26758, -41.97364, 40.923637, 34.426544, -53.40543, -3.7035291, -32.267063, -68.91373, 97.09721, -34.846046, 3.907564, -66.201614, -118.9858, 105.06361, 178.53958, 4.0544424, -94.79694, -20.102713, -98.62863, -39.861652, -13.361424, -4.9815593, -166.14812, -37.12321, 5.371111, 36.828648, -90.45125, -107.01102, 131.4
.72582, -106.29445, -107.62907, -128.25266, 57.016, -4.0009613, 68.63145, 166.55951, 16.245739, 7.967949, 55.68995, 203.96786, 126.397316, -13.356705, -102.254875, -19.872458, 26.919744, 39.288116, 63.586567, 84.60314, 45.755398, -164.70078, -37.978497, -108.65916, 8.707288, -92.352715, -12.429333, -14.801342, -20.504671, 44.00929, 36.68336, 52.246624, 130.94351, 14.492783, -57.93018, -67.071526, 14.621403, -138.22528, -2.6102273, -121.38736, -113.420975, -61.893253, -98.235664, -62.990517, -26.994469, -30.541307, -25.734997, -86.89197, -40.087112, 9.094153, -83.56758, -20.74797, -9.748177, 1.2632121, -13.608057, -6.9518933, 24.433912, 5.579871, 118.29234, -56.721317, 21.968445, 134.80156, -39.210865, -9.40181, 6.156143, -85.19102, 111.48763, 21.58796, 122.76232, -90.41675, 9.152984, -165.5826, 40.304283, -8.051397, 66.11045, -4.1537495, 7.916171, -96.71719, -32.474934, -63.480034, 111.893, -65.39603, 86.92193, -20.23148, -29.124113, -6.574574, 120.03416, -43.831738, 59.963367, 20.325058, -57.137333, -23.18118, -72.90113, -2.1142719, -48.737312, 33.665985, -31.749983, -2.3302555, 18.836487, -76.74881, 63.42666, 9.605455, -174.03116, 68.61991, -110.84995, 15.163495, -42.632076, -18.957863, -168.69986, 100.11529, -40.10067, -65.51679, 59.719948, -146.70041, -114.89861, -82.28902, -49.428722];
    ...
    let all_tracks_dirs: AllTrackDirs = serde_json::from_str(&std::fs::read_to_string("graphs/tracks_dirs.json").unwrap()).unwrap();

    params_sim.nn.use_dirs_autoencoder = true;
    params_sim.nn.autoencoder_exits = 21;
    params_sim.nn.autoencoder_hidden_layers = vec![];
    let nn_ae_true = (0..22).flat_map(|x| (0..21).map(move |y| if y == x { 1.0 } else { 0. })).chain((0..22).flat_map(|x| (0..21).map(move |y| if y == x { 1.0 } else { 0. }))).collect::<Vec<f32>>();

    {
        ...
        let nn_keras: NeuralNetworkUnoptimized = serde_json::from_str(&std::fs::read_to_string("graphs/neural_network_10_10_10_5.json").unwrap()).unwrap();

        let nn_input = NeuralNetworkUnoptimized { layers: nn_keras.layers.iter().cloned().take(4).collect() }.to_optimized();
        let nn_output = NeuralNetworkUnoptimized { layers: nn_keras.layers.iter().cloned().skip(4).collect() }.to_optimized();

        params_sim.nn.autoencoder_hidden_layers = vec![10, 10, 10];
        params_sim.nn.autoencoder_exits = 5;
        let nn_ae = nn_input.get_values().iter().chain(nn_output.get_values().iter()).copied().collect::<Vec<f32>>();
        ...
        let (params_input, params_output) = nn_ae.split_at(nn_params.get_nn_autoencoder_input_len());
        let mut nn_input = NeuralNetwork::new_params(nn_params.get_nn_autoencoder_input_sizes(), params_input);
        let mut nn_output = NeuralNetwork::new_params(nn_params.get_nn_autoencoder_output_sizes(), params_output);
    ...
    let converted = all_tracks_dirs.0.iter().flat_map(|x| x.0.iter()).map(|x| x.0.iter().map(|y| convert_dir(&params_sim.nn, *y)).collect::<Vec<f32>>()).collect::<Vec<Vec<f32>>>();
    dbg!(converted.len());
